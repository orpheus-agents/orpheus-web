...
* Design rules live in `branding/interface.md` and `branding/colors.md`; follow them instead of restating them here. Colours and surfaces come only from the semantic Tailwind tokens defined in `src/styles/main.css` (CSS variables for both themes); do not hardcode classes such as `bg-white` or `text-amber-800`. Reuse the classes in `src/styles/main.css` (`section-title`, `panel-title`, `eyebrow`, `caps`, `button`, `field`, `data-table`, `marker`, `console`) instead of restyling elements in templates.
