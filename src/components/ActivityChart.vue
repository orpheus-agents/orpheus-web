...
const color = {
  [RunStatus.completed]: 'var(--color-brand)',
  [RunStatus.failed]: 'var(--color-danger)',
  [RunStatus.cancelled]: 'var(--color-warning)',
  [RunStatus.running]: 'var(--color-muted)',
}
...
<template>
  <section class="panel overflow-hidden">
    <div class="flex flex-wrap items-center justify-between gap-4 px-6 pt-6">
      <div>
        <h2 class="font-semibold">{{ t('analytics.activity') }}</h2>
        <p class="mt-1 text-xs text-muted">{{ t('analytics.chartHint', { timezone: overview.timezone }) }}</p>
      </div>
      <div class="flex flex-wrap gap-4 text-xs text-muted">
        <span v-for="status in groups" :key="status" class="flex items-center gap-1.5"><span class="h-2 w-2 rounded-sm" :style="{ background: `rgb(${color[status]})` }" />{{
          t(status === RunStatus.running ? 'analytics.inProgress' : `status.${status}`)
        }}</span>
      </div>
    </div>
    <div class="px-3 pb-2 pt-5 sm:px-6">
      <svg
        ref="plot"
        class="h-64 w-full"
        :viewBox="`0 0 ${width} 250`"
        role="img"
        :aria-label="t('analytics.activity')"
      >
        <g v-for="part in [0, 0.5, 1]" :key="part">
          <line
            x1="45"
            :x2="width - 10"
            :y1="210 - part * 190"
          ...
          />
          <text x="32" :y="215 - part * 190" text-anchor="end" fill="rgb(var(--color-muted))" font-size="11">
            {{ formatNumber(Math.round(max * part), locale) }}
          </text>
        </g>
        <g v-for="bar in bars" :key="bar.bucket.from" @pointerenter="selected = bar.index">
          <rect
            :x="45 + bar.index * step"
            y="20"
            :width="step"
      ...
      <input
        v-if="overview.series.length"
        v-model.number="selected"
        class="mt-2 w-full accent-brand"
        type="range"
        min="0"
        :max="overview.series.length - 1"
        :aria-label="t('analytics.selectInterval')"
      >
    </div>
    <div
      v-if="current"
      class="flex min-h-16 flex-wrap items-center justify-between gap-3 border-t border-line bg-surface/50 px-6 py-4 text-xs"
    >
